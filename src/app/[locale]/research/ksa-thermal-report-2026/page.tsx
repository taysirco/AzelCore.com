...
const PATH = '/research/ksa-thermal-report-2026';

// The report body is English — it is a research artifact citing international
// standards — but the route now lives under [locale] like every other page, so
// title/description follow the visitor's locale and canonical/hreflang are
// generated by the same helper the rest of the site uses.
export async function generateMetadata(
  ...
  const title = isAr
    ? 'تقرير حجب الحرارة وتوفير التكييف في السعودية 2026 — أبحاث عزل كور'
    : 'KSA Thermal Rejection & HVAC Energy Report 2026 — AzelCore Research';

  const description = isAr
    ? 'بيانات ميدانية من مصدر أولي: حجب الحرارة داخل المركبات عبر 10 أحياء بجدة (FLIR T530، حجم العينة 530). وقياسات خفض أحمال التكييف في المباني وفق كود البناء السعودي SBC 601/602. بيانات مفتوحة للاستشهاد.'
    : 'Primary-source field data: Vehicle interior thermal rejection across 10 Jeddah districts (FLIR T530, n=530). Building HVAC load reduction measurements per SBC 601/602. Open dataset for citation.';

  return {
    title,
    description,
    alternates: getAlternates(locale as Locale, PATH),
    openGraph: {
      title: isAr
        ? 'تقرير حجب الحرارة 2026 — دراسة ميدانية من عزل كور'
        : 'KSA Thermal Rejection Report 2026 — AzelCore Field Research',
      description: isAr
        ? 'بيانات أداء حراري موثّقة بكاميرا FLIR لأفلام النانو سيراميك في السعودية. 530 عيّنة مركبة عبر 10 أحياء بجدة.'
        : 'FLIR-validated thermal performance data for nano-ceramic window films in Saudi Arabia. 530 vehicle samples across 10 Jeddah districts.',
      url: `${SITE_URL}${localePath(locale as Locale, PATH)}`,
      type: 'article',
    },
  };
}
...
// Built per locale so each localized page's graph points at itself, matching the
// self-referencing canonical that getAlternates emits. hreflang ties the two together.
const buildResearchGraph = (locale: Locale) => {
  const PAGE_URL = `${SITE_URL}${localePath(locale, PATH)}`;
  const HOME_URL = `${SITE_URL}${localePath(locale, '/')}`;
  ...
  '@graph': [
    ...
    },
    {
      '@type': 'BreadcrumbList',
      itemListElement: [
        { '@type': 'ListItem', position: 1, name: locale === 'ar' ? 'الرئيسية' : 'Home', item: HOME_URL },
...
export default async function KsaThermalReportPage(
  { params }: { params: Promise<{ locale: string }> }
) {
  ...
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(buildResearchGraph(locale)) }} />

      {/* The report body is English in both locales — it cites international standards
          and is written for citation. Declaring lang/dir on the article itself keeps the
          English text left-to-right even when the surrounding document is Arabic/RTL,
          which the old standalone layout used to handle by forcing lang="en" on <html>. */}
      <article lang="en" dir="ltr" style={{ maxWidth: '900px', margin: '0 auto', padding: '2rem 1.5rem', fontFamily: 'system-ui, sans-serif', lineHeight: 1.8, color: 'var(--text)', textAlign: 'start' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem', marginTop: '1rem' }}>
              ...
              <tbody>
                {vehicleData.map((d, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #333' }}>
                    ...
                    <td>{d.zone}</td>
                    <td>{d.ambientC}</td>
                    <td style={{ color: 'var(--danger)' }}>{d.noTintC}</td>
                    <td style={{ color: 'var(--success)' }}>{d.nanoTintC}</td>
              ...
              <tfoot>
                <tr style={{ borderTop: '2px solid #555', fontWeight: 700 }}>
                  ...
                  <td style={{ color: 'var(--danger)' }}>{(vehicleData.reduce((a, d) => a + d.noTintC, 0) / vehicleData.length).toFixed(1)}</td>
                  <td style={{ color: 'var(--success)' }}>{(vehicleData.reduce((a, d) => a + d.nanoTintC, 0) / vehicleData.length).toFixed(1)}</td>
              ...
              <tbody>
                {buildingData.map((b, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #333' }}>
                    ...
                    <td style={{ color: 'var(--danger)' }}>{b.preKwh.toLocaleString()}</td>
                    <td style={{ color: 'var(--success)' }}>{b.postKwh.toLocaleString()}</td>
              ...
              <tbody>
                {uvDegradationData.map((u, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #333' }}>
                    ...
                    <td style={{ color: 'var(--danger)' }}>{u.noFilm6mo}</td>
                    <td style={{ color: 'var(--success)' }}>{u.nanoFilm6mo}</td>
        ...
        <section style={{ marginTop: '2rem' }}>
          ...
          <ul>
            <li><strong>JSON API:</strong> <a href={`${SITE_URL}/api/research/jeddah-thermal-data`} style={{ color: 'var(--primary-light)' }}>{SITE_URL}/api/research/jeddah-thermal-data</a></li>
            <li><strong>OpenAPI Spec:</strong> <a href={`${SITE_URL}/openapi.json`} style={{ color: 'var(--primary-light)' }}>{SITE_URL}/openapi.json</a></li>
