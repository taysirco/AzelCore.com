 ...
 */
export default function CopyButton({ valueToCopy, label }: CopyButtonProps) {
  const params = useParams();
  const isAr = params.locale === 'ar';
  const finalLabel = label || (isAr ? 'نسخ' : 'Copy');
  ...
  return (
    <button 
      ...
      aria-label={isAr ? `نسخ ${valueToCopy}` : `Copy ${valueToCopy}`}
    >
      {copied ? (
        <>
